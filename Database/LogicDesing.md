# ¿ Que es una Base de datos?
son tablas de informacion o de archivos almacenados que se utiliza para trabajos para facilitar el trabajo de identificacion a atravez de un id unico para cada persona y otro para las mascotas 

ejm: antonio id=31684-a y para juan (osea la mascota) id: 21786-b





## ¿ Porque es mejor usar float que int en el ingreso de la edad
Porque el int puede poner numeros negativos si la cantidad de numeros es demasiado alta (osea redondiado)

ejm: en minecraft los numeros de los bloques esta en int lo que singnifica que en servidores anarquicos como 2b2t pueden utilizar bugs y gliches del juego para duplicar bloques del juego y tener tantos bloques que el sistema para no colapsar pone numeros negativos




### ¿Por que ingresar edad en un sistema de base de datos y cual seria una mejor opcion en este caso justifque ?




###  ¿El concepto de las bases de datos son un programa eso es verdad?
tanto la base de datos como las aplicasiones son un conjunto de sprits que sirven como bloque de legos para generar un programa o una base de datos 

(veo que esta pregunta ahora no tiene puntaje por el momento :( )

## El "Dato" vs. "Realidad": En la pizarra se propuso usar `FLOAT` con el valor `0.5` para representar a un cachorro de 6 meses. Si el sistema debe enviar un saludo automático de "Feliz Cumpleaños" cada año, ¿qué problema técnico enfrentaríamos si solo guardamos `0.5` y no la fecha de nacimiento (`DATE`)? 
si la fecha de naciomiento si se perdiese atraves de pantalla azul de la muerte y por la deconetcion por la red por falta de wifi por que el codigo al no tener la fecha de nacimietos el sistema puede que piense que el valor 


## foto de la tabla


<img width="4160" height="3120" alt="IMG_20260511_094535" src="https://github.com/user-attachments/assets/419ad079-6480-407e-b99e-8752d968d423" />
